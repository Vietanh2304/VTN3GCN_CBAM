"""
Original File: tools/gen_pose_v2.py
Adapted For: MM-WLAuslan (3 views: center/left/right)
Model: rtmpose-m_8xb512-700e_body8-halpe26-256x192
Outputs: 26 keypoints (Halpe-26 format), keys: raw_pose, pose_threshold_02, prob
"""
...
def gen_pose(video_url, kp_folder, pose_detector, file_name):
    ...
    try:
        # MMPoseInferencer generates results as a generator
        pose_results = pose_detector(video_url)
        for idx, pose_result in enumerate(pose_results):
            if 'predictions' not in pose_result or len(pose_result['predictions']) == 0 or len(pose_result['predictions'][0]) == 0:
                continue
...
def main():
    parser = argparse.ArgumentParser(description='Extract poses for MM-WLAuslan')
    ...
    parser.add_argument('--output_dir', type=str, required=True, help='Output directory for JSONs')
    ...
    with tqdm(total=total_tasks, desc=f"Shard {args.shard}/{args.num_shards}") as pbar:
        for idx, row in df.iterrows():
            for view in views:
                ...
                gen_pose(video_url, kp_folder, pose_detector, file_name)
                pbar.update(1)
